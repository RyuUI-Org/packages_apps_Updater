...
updates_dir=/data/ryu_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have RyuUI-VERSION-DATE-TYPE-* as format"
...
# Assume RyuUI-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.ryu.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.ryu.updater/databases/updates.db" \
